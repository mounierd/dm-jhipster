<template>
  <div>
    <h2 id="page-heading" data-cy="CarHeading">
      <span v-text="$t('blogApp.car.home.title')" id="car-heading">Cars</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('blogApp.car.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CarCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-car">
            ...
            <span v-text="$t('blogApp.car.home.createLabel')"> Create a new Car </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && cars && cars.length === 0">
      <span v-text="$t('blogApp.car.home.notFound')">No cars found</span>
    </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="blogApp.car.delete.question" data-cy="carDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-car-heading" v-text="$t('blogApp.car.delete.question', { id: removeId })">
